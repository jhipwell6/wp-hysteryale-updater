...
namespace HysterYale\Updater\Models\Abstracts;
...
abstract class Post_Model
{
	...
	{
		return array_merge( $this->hidden, array(
			'post_type',
			'unique_key',
			'post_object',
			'aliases',
	...
	public function create( $args = array() )
	{
		...
		$args['post_content'] = is_callable( array( $this, $content_getter ) ) && $this->{$content_getter}() != '' ? $this->{$content_getter}() : ' ';
		...
		if ( is_callable( array( $this, $date_getter ) ) ) {
			$args['post_date'] = $this->{$date_getter}();
			$args['post_date_gmt'] = get_gmt_from_date( $this->{$date_getter}() );
		}
	...
	private function map_properties( $props )
	{
		...
		$props = $this->convert_prop_case( $props );

		return $props;
	}

	private function convert_prop_case( $props )
	{
		$new_props = array();
		foreach ( $props as $key => $value ) {
			$new_key = $this->decamelize( $key );
			if ( is_array( $value ) ) {
				$new_props[$new_key] = $this->convert_prop_case( $value );
			} else {
	...
	private function is_wp_prop( $prop )
	{
	...
	{
		if ( is_array( $value ) ) {
			array_walk_recursive_array( $value, array( $this, 'sanitize_value' ) );
		}

		return hygpm_decode( $value );
	}
